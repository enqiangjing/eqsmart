...
class Application:
    def __init__(self, path):
        ...
        self.conf_path = path
        ''' Provider服务列表扫描 '''
        self.services = scan_services.get_services_new()
        self.services_class = scan_services.get_services_class_new()
        self.provider_conf = read_components_module.read('load_provider_conf').ProviderServerConf(path).read_server()
        ''' HTTP服务列表扫描 '''
        self.http_services = scan_http_services.get_services_new()
        self.http_services_class = scan_http_services.get_services_class_new()
